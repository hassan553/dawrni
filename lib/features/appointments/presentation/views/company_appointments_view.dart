...
class CompanyAppointmentsView extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Padding(
      ...
      // child: BlocListener<DeleteCompanyAppointmentBloc, BaseState<void>>(
      ...
      //     }
      //   },
        child: BlocBuilder<CompanyAppointmentsBloc, BaseState<CompanyAppointmentsEntity>>(
        builder: (context, state) {
          if (state.isLoading) {
            return const LoadingComponent();
          } else if (state.isError) {
            return FailureComponent(failure: state.failure, retry: () => getCompanyAppointments(context));
          } else if (state.isSuccess) {
            return SmartRefresher(
              controller: context.read<CompanyAppointmentsBloc>().refreshController,
              onRefresh: () => getCompanyAppointments(context, refresh: true),
              ...
              child: ListView(
                children: [
                  ...
                  ...state.data?.appointments.map((e) => CompanyAppointmentCard(appointment: e)).toList() ?? [],
